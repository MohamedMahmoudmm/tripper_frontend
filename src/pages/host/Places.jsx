...
const Places = () => {
  ...
  useEffect(() => {
    axiosInstance.get("/places").then((res) => {
      ...
      // Group places by city for carousel sections
      const grouped = placesData.reduce((acc, place) => {
        ...
        if (!acc[placeCity]) acc[placeCity] = [];
        acc[placeCity].push(place);
        return acc;
      }, {});
      ...
      if (placesData.length > 0) {
        console.log("First Place:", placesData[0]);
      }
    }).catch(err => {
  ...
  return (
    <Box>
      ...
      {/* Popular Places Carousel Sections - في الآخر */}
      {Object.keys(groupedByCity).map((city) => (
        <PopularHomesCarousel
          key={city}
          homes={groupedByCity[city].map(place => ({
            image: place.images?.[0] || "",
            title: place.name,
            price: place.description,
            rating: place.starRating || 0,
            model: "places",
            id: place._id
          }))}
          title={`Popular Places in ${city}`}
          city={city}
        />
      ))}
    </Box>
